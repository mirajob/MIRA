      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-left text-eyebrow text-navy/60 uppercase py-3 px-4">Associazione</th>
          ...
          {invitations.map((inv) => (
            <tr key={inv.id} className="border-b border-border last:border-0 hover:bg-navy-50/50">
              ...
              <td className="py-4 px-4 text-body text-ink">
                {(inv.metadata as Record<string, string>)?.association_name ?? "—"}
              </td>
              ...
              <td className="py-4 px-4 text-right">
                {inv.status === "pending" && (
                  <form action={() => revokeInvitation(inv.id)}>
                    <button
                      type="submit"
