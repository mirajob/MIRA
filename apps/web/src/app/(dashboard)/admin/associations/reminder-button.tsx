...
import { remindAssociationPage } from "@/lib/actions/admin-notify";

export function ReminderButton({ associationId, associationName }: { associationId: string; associationName: string }) {
  const t = useTranslations("AdminAssociations");

  const [open, setOpen] = useState(false);
  const [subject, setSubject] = useState(() => t("reminderSubjectDefault"));
  const [message, setMessage] = useState(() => t("reminderMessageDefault", { name: associationName }));
  ...
  async function handleSend() {
    setLoading(true);
    setResult(null);
    const res = await remindAssociationPage({ associationId, subject, message });
    setResult(res);
    setLoading(false);
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <p className="text-body-sm text-ink-secondary mb-4">{associationName}</p>
