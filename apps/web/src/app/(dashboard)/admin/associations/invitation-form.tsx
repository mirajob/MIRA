...
export function InvitationForm() {
  ...
  return (
    <div className="rounded-lg border border-border bg-white p-4">
      <h2 className="font-sans text-body font-semibold text-navy mb-1">{t("inviteHeading")}</h2>
      ...
      <form action={handleSubmit} className="grid gap-3 sm:grid-cols-2">
        <label className="block">
          <span className="text-eyebrow uppercase text-navy mb-1 block">{t("presidentEmailLabel")}</span>
          <input
            name="email"
            type="email"
            required
        ...
        <label className="block">
          <span className="text-eyebrow uppercase text-navy mb-1 block">{t("associationNameLabel")}</span>
          <input
