  ...
  return (
    <div className="space-y-5">
      ...
      {(accessRequests?.length ?? 0) > 0 && (
        <section>
          <h2 className="font-sans text-body font-semibold text-navy mb-2">{t("accessRequestsHeading", { count: accessRequests.length })}</h2>
            ...
            <div className="overflow-x-auto"><table className="w-full min-w-[760px]">
              <thead>
                <tr className="border-b border-border bg-amber-50">
                  <th className="px-3 py-2 text-left text-eyebrow uppercase text-navy/60">{t("tableCompany")}</th>
                  <th className="px-3 py-2 text-left text-eyebrow uppercase text-navy/60">{t("tableContact")}</th>
                  <th className="px-3 py-2 text-left text-eyebrow uppercase text-navy/60">{t("tableWebsite")}</th>
                ...
                {accessRequests.map((r: any) => (
                  <tr key={r.id} className="border-b border-border last:border-0 hover:bg-paper transition-colors">
                    ...
                    <td className="px-3 py-2">
                      ...
                      <p className="text-eyebrow text-ink-tertiary">{r.email}</p>
                    </td>
                    <td className="px-3 py-2">
                      {r.website_url ? (
      ...
      {pending.length > 0 && (
        <section>
          <h2 className="font-sans text-body font-semibold text-navy mb-2">{t("pendingApprovalHeading", { count: pending.length })}</h2>
          <div className="rounded-lg border border-amber-200 bg-white overflow-hidden">
            <div className="overflow-x-auto"><table className="w-full min-w-[760px]">
              <thead>
                <tr className="border-b border-border bg-amber-50">
      ...
      {active.length > 0 && (
        <section>
          <h2 className="font-sans text-body font-semibold text-navy mb-2">{t("activeCompaniesHeading", { count: active.length })}</h2>
      ...
      {!companies?.length && (
        <div className="rounded-lg border border-border bg-white p-6 text-center">
          <p className="text-body-sm text-ink-secondary">{t("noCompanies")}</p>
        </div>
      )}
    </div>
  );
