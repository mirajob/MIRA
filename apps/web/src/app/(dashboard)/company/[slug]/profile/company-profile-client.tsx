  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-h1 text-navy">Profilo aziendale</h1>
        {!editing ? (
          <button
      ...
      <div className="rounded-lg border border-border bg-white p-6 space-y-6">
        ...
        <div>
          <p className="text-label text-ink-secondary mb-1">Ragione sociale</p>
          <p className="text-body text-ink">{company.legal_name}</p>
          <p className="text-body-sm text-ink-tertiary mt-0.5">Non modificabile</p>
        </div>

        {/* Display name */}
        <div>
          <p className="text-label text-ink-secondary mb-2">Nome visualizzato</p>
          {editing ? (
        ...
        {/* Sector */}
        <div>
          <p className="text-label text-ink-secondary mb-2">Settore</p>
          {editing ? (
            ...
            <p className="text-body text-ink">{company.sector ?? <span className="text-ink-tertiary italic">Non specificato</span>}</p>
          )}
        </div>

        {/* Website */}
        <div>
          <p className="text-label text-ink-secondary mb-2">Sito web</p>
          {editing ? (
            ...
            <p className="text-body text-ink-tertiary italic">Non specificato</p>
          )}
        </div>

        {/* Description */}
        <div>
          <p className="text-label text-ink-secondary mb-2">Descrizione</p>
          {editing ? (
            <textarea rows={4} value={draft.description} onChange={(e) => setDraft((p) => ({ ...p, description: e.target.value }))}
              placeholder="Descrivi brevemente l'azienda, cosa fate, la cultura…"
            ...
            <p className="text-body text-ink whitespace-pre-wrap">{company.description ?? <span className="text-ink-tertiary italic">Nessuna descrizione</span>}</p>
          )}
        </div>

        {/* Status */}
        <div>
          <p className="text-label text-ink-secondary mb-1">Stato account</p>
